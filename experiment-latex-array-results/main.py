...
def applyAlgo(algo, p, Xtrain, Ytrain, Xtest, Ytest):
    ...
    if algo.startswith("GFC;"):
        nbIter = int(algo.split(";")[1])
        clf = GreedyFeatureConstruction(numFeatures=nbIter, C=p["C"],
                                        randomState=np.random.RandomState(1))
...
listParams = {}
parBMKR = listP({"C": np.logspace(-2, 2, 5)})
...
parPBRFF = listP({"gamma": [2**i for i in [-2, -1, 0, 1, 2]],
                  ...
                  "C": np.logspace(-2, 2, 5)})
parGFC = listP({"C": np.logspace(-2, 2, 5)})
parGBRFF1 = listP({"gamma": [2**i for i in [-2, -1, 0, 1, 2]],
                   "beta": np.logspace(-2, 2, 5)})
parGBRFF2 = listP({"Lambda": [0] + [2**i for i in [-5, -4, -3, -2]],
                   "gamma": [2**i for i in [-2, -1, 0, 1, 2]]})

listParams["BMKR;100"] = parBMKR
listParams["LGBM;100"] = parLGBM
listParams["PBRFF;100;10"] = parPBRFF
listParams["GFC;100"] = parGFC
listParams["GBRFF1;100;10"] = parGBRFF1
listParams["GBRFF2;100"] = parGBRFF2
